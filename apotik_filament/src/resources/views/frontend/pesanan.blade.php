      ...
      @forelse ($pesanans as $pesanan)
        <div class="card border-0 shadow-sm mb-3">
          <div class="card-body d-flex justify-content-between align-items-center">
            ...
            <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#pesananModal{{ $pesanan->id }}">
              Lihat Detail
            </button>
          </div>
        </div>
        ...
        <!-- Modal Detail -->
        <div class="modal fade" id="pesananModal{{ $pesanan->id }}" tabindex="-1" aria-labelledby="modalLabel{{ $pesanan->id }}" aria-hidden="true">
          <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="modalLabel{{ $pesanan->id }}">
                  Detail {{ $pesanan->pengajuan_id ? 'Pengajuan' : 'Pesanan' }} #{{ $pesanan->nomor_pesanan }}
                </h5>
              ...
              <div class="modal-body p-4">
                ...
                {{ $item->obat->nama_obat ?? '(Obat tidak ditemukan)' }} x{{ $item->qty }}
